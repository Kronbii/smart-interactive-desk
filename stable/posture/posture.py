    ...
    try:
        with mp_holistic.Holistic(
            ...
            while cap.isOpened():
                ...
                cv2.resizeWindow("Sign Language Detection", x, y)
                ...
                up_threshold = y/2 + 70
                down_threshold = y/2 - 70
                
                if shoulder_distance is None:
                    print("stop")
                    return "s", False
                elif shoulder_distance > up_threshold:
                    print("up")
                    return "u", True
                elif shoulder_distance < down_threshold:
                    print("down")
                    return "d", True
                else:
                    print("stop")
                ...
                cv2.line(image, (0, int(y/2)), (x, int(y/2)), (0, 0, 255), 2)
    
                # Draw landmarks
                draw_landmarks(image, results)
    
                # Resize the image to 1800x1200 before displaying
                image_resized = cv2.resize(image, (x, y))
